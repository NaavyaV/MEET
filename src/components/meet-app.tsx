...
import { isEligibleForProfile } from "@/src/lib/engine";
...
type NetworkConnection = {
  ...
  createdAt: string;
  person: { id: string; fullName: string; username: string | null; avatarUrl: string | null };
};
...
function EventCard({ event, action, onSelect, onAction }: { event: Opportunity; action?: EventAction; onSelect: () => void; onAction: (action: EventAction | null) => void }) {
  const date = dateParts(event.startsAt);
  const score = event.score?.final ?? 0;
  const scoreTone = score >= 7.5 ? "excellent" : score >= 5.5 ? "good" : "low";
  const fitLabel = score >= 7.5 ? "Strong match" : score >= 5.5 ? "Worth exploring" : "Broader match";
  return <article className="event-card">
    <button className="event-main" onClick={onSelect} aria-label={`Open ${event.title}`}>
      ...
      <div className="event-copy">
        ...
        <p className="event-reason">{score < 6.3 ? event.score?.lowScoreExplanation : event.distanceMiles == null || event.format === "online" ? "Fits your preferred format" : `Within your ${event.distanceMiles <= 5 ? "nearby" : "selected"} travel area`}</p>
        <FriendLine names={event.friendNames} />
      </div>
      <span className={`score-mark ${scoreTone}`} aria-label={`${score.toFixed(1)} out of 10 match`}><b>{score.toFixed(1)}</b><span>/10</span><small>{fitLabel}</small></span>
    </button>
    <div className="card-actions">
      <button className={action === "saved" ? "pressed" : ""} onClick={() => onAction(action === "saved" ? null : "saved")} title="Save event"><Icon name="bookmark" size={16} />{action === "saved" ? "Saved" : "Save"}</button>
...
function EventDetail({ event, action, onClose, onAction }: { event: Opportunity; action?: EventAction; onClose: () => void; onAction: (action: EventAction | null) => void }) {
  const date = dateParts(event.startsAt);
    ...
    <section className="event-detail" role="dialog" aria-modal="true" aria-label={event.title} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="score-panel fit-summary">
        ...
        <div className="fit-summary-grid">
          ...
          <article><span>How it is ordered</span><p>MEET ranks nearby opportunities by relevance and verified proximity only.</p></article>
        </div>
...
export function MeetApp() {
  ...
  const persistEventAction = async (event: Opportunity, action: EventAction | null) => {
    const supabase = createSupabaseBrowserClient();
    const session = supabase ? (await supabase.auth.getSession()).data.session : null;
    if (!session || event.sourceType === "demo") return;
    try {
      const response = await fetch("/api/account/preferences", { method: action ? "PUT" : "DELETE", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ event: { id: event.id, url: event.url }, ...(action ? { action } : {}) }) });
  ...
  const updateAction = (event: Opportunity, action: EventAction | null) => { setActions((current) => { const next = { ...current }; if (action) next[event.id] = action; else delete next[event.id]; window.localStorage.setItem("meet-actions", JSON.stringify(next)); return next; }); setToast(action === "going" ? `You’re going to ${event.title}.` : action === "saved" ? `${event.title} saved.` : `${event.title} removed from saved events.`); void persistEventAction(event, action); };
  ...
  // Defense in depth for cached refreshes: never render a physical event whose
  // distance is missing or outside the member's current travel area.
  const filtered = useMemo(() => events.filter((event) => isEligibleForProfile(event, profile) && actions[event.id] !== "dismissed" && (filter === "all" || event.category === filter) && (format === "all" || event.format === format)), [events, profile, actions, filter, format]);
  const top = filtered.filter((event) => (event.score?.final ?? 0) >= 6.3); const low = filtered.filter((event) => (event.score?.final ?? 0) < 6.3);
...
function Network({ authenticated, onSignIn }: { authenticated: boolean; onSignIn: () => void }) {
  ...
  const loadConnections = useCallback(async () => {
    const supabase = createSupabaseBrowserClient();
    const session = supabase ? (await supabase.auth.getSession()).data.session : null;
    if (!session) { setConnections([]); return; }
    const response = await fetch("/api/account/network", { headers: { Authorization: `Bearer ${session.access_token}` } });
    const payload = await responseJson<{ connections?: NetworkConnection[]; error?: string }>(response);
    if (!response.ok) throw new Error(payload.error || "Could not load your network.");
    setConnections(payload.connections ?? []);
  }, []);

  useEffect(() => {
    if (!authenticated) return;
    const task = window.setTimeout(() => { void loadConnections().catch(() => undefined); }, 0);
  ...
  const sendRequest = async () => {
    ...
    const supabase = createSupabaseBrowserClient();
    const session = supabase ? (await supabase.auth.getSession()).data.session : null;
    if (!session || !supabase) { onSignIn(); return; }
    setLoading(true); setNote("");
    try {
      const { error } = await supabase.rpc("request_connection_by_identifier", { identifier: identifier.trim() });
      if (error) throw new Error(error.message);
      setIdentifier(""); setNote("Connection request sent."); await loadConnections();
    } catch (error) { setNote(error instanceof Error ? error.message : "Could not send the connection request."); }
    finally { setLoading(false); }
  };

  const changeConnection = async (connection: NetworkConnection, method: "PATCH" | "DELETE") => {
    const supabase = createSupabaseBrowserClient();
    const session = supabase ? (await supabase.auth.getSession()).data.session : null;
    if (!session) { onSignIn(); return; }
    setLoading(true); setNote("");
    try {
      const response = await fetch("/api/account/network", { method, headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify(method === "PATCH" ? { connectionId: connection.id, status: "accepted" } : { connectionId: connection.id }) });
      const payload = await responseJson<{ error?: string }>(response);
      if (!response.ok) throw new Error(payload.error || "Could not update that connection.");
      setNote(method === "PATCH" ? `${connection.person.fullName} is now in your network.` : "Connection removed."); await loadConnections();
    } catch (error) { setNote(error instanceof Error ? error.message : "Could not update that connection."); }
    finally { setLoading(false); }
  };

  const accepted = connections.filter((connection) => connection.status === "accepted");
  const incoming = connections.filter((connection) => connection.status === "pending" && connection.direction === "received");
  const outgoing = connections.filter((connection) => connection.status === "pending" && connection.direction === "sent");
  const card = (connection: NetworkConnection, index: number) => {
    const initials = connection.person.fullName.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase();
    return <article className="friend-card" key={connection.id}><span className={`friend-avatar f${index % 3}`}>{initials}</span><div><h3>{connection.person.fullName} {connection.person.username && <small>@{connection.person.username}</small>}</h3><p><span className="going-dot" />{connection.status === "accepted" ? "Connected on MEET" : connection.direction === "received" ? "Wants to connect" : "Request pending"}</p></div>{connection.status === "pending" && connection.direction === "received" ? <button className="text-button" disabled={loading} onClick={() => void changeConnection(connection, "PATCH")}>Accept</button> : <button className="text-button" disabled={loading} onClick={() => void changeConnection(connection, "DELETE")}>{connection.status === "accepted" ? "Remove" : "Cancel"}</button>}</article>;
  };

  return <div className="page network-page"><header className="page-title"><div><span className="eyebrow">Your small, useful network</span><h1>Go where your people are.</h1><p>Connections are private and only power attendance context — no feed, no noise.</p></div></header><section className="network-add"><div><span className="eyebrow">Add a connection</span><h2>Know someone who should be in the room?</h2></div><div className="add-form"><input value={identifier} onChange={(event) => setIdentifier(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void sendRequest(); }} placeholder="Email or username" disabled={loading} /><button className="button" onClick={() => void sendRequest()} disabled={loading}>{loading ? "Working…" : authenticated ? "Send request" : "Sign in to connect"}</button></div></section>{note && <p className="network-note">{note}</p>}{!authenticated ? <section className="network-empty"><h2>Sign in to build your network.</h2><p>Connection requests, accepted connections, and attendance context are saved privately to your MEET account.</p><button className="button" onClick={onSignIn}>Sign in</button></section> : <><section className="friends-list"><div className="section-heading"><div><span className="eyebrow">Accepted connections</span><h2>{accepted.length ? `${accepted.length} ${accepted.length === 1 ? "person" : "people"} in your network` : "Your network is ready when you are."}</h2></div></div>{accepted.length ? accepted.map(card) : <div className="empty-state">Send a request by email or username to add a private connection.</div>}</section>{incoming.length > 0 && <section className="friends-list pending-list"><div className="section-heading"><div><span className="eyebrow">Requests to you</span><h2>Waiting for your response</h2></div></div>{incoming.map(card)}</section>}{outgoing.length > 0 && <section className="friends-list pending-list"><div className="section-heading"><div><span className="eyebrow">Sent requests</span><h2>Waiting for a response</h2></div></div>{outgoing.map(card)}</section>}</>}<section className="network-privacy"><Icon name="info" /><div><b>Attendance is visible only to accepted connections.</b><p>MEET keeps your network and event activity private with Supabase Row Level Security.</p></div></section></div>;
}
