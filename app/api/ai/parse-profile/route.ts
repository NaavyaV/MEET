...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? `We couldn't read that file: ${error.message}` : "We couldn't read that file. Please try another PDF or enter your details manually." }, { status: 422 });
  }
}
